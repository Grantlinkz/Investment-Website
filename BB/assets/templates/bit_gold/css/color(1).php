
.base--color,.header .main-menu li a:hover, .header .main-menu li a:focus,.page-breadcrumb li:first-child::before,.page-breadcrumb li a:hover,.d-widget .icon{
    color: #d03149 !important;
}

.border-btn:hover, .cmn-btn, .cmn-btn:hover,.base--bg, .team-card:hover, .subscribe-form .subscribe-btn, .pagination li.disabled .page-link,.pagination li .page-link:hover, .pagination li.active .page-link, .contact-item, .header .main-menu li .sub-menu,.d-widget,.investor-card:hover,.blog-details__footer .social__links li a:hover,.sidebar .widget .widget__title::after,select option, .account-menu .icon i,.account-menu .icon .account-submenu{
    background-color: #d03149 !important;
}

.border-btn:hover,.package-card, .page-item.disabled .page-link,.pagination li .page-link:hover, .pagination li.active .page-link {
    border-color: #d03149;
}

.investor-card:hover {
    box-shadow: 0 5px 10px 5px #d0314980;
}
.package-card, .package-card:hover {
    box-shadow: 0 5px 15px #d0314980;
}

.package-card__features li {
    border-bottom: 1px solid #d0314959;
}
.profit-calculator-wrapper {
    border: 2px solid #d0314980;
    box-shadow: 0 0 15px #d0314980;
}
select {
    border: 1px solid #d0314973;
}
input[type="text"]:read-only, input[type="email"]:read-only, input[type="text"]:disabled, input[type="email"]:disabled {
    border-color: #d03149;
}

.work-card__icon {
    border: 3px solid #d03149;
    box-shadow: 0 0 15px 3px #d03149a6;
}

.work-card__icon .step-number {
    border: 2px solid #d03149;
}
.work-item::before {
    border-top: 1px dashed #d03149;
}

.cmn-accordion .card-header .btn {
    background-color: #d03149;
}

.testimonial-card {
    background-color: #d03149a6;
    box-shadow: 0 5px 0px #d03149;
}

.card .card-header {
    background-color: #d03149;
}

.cmn-accordion .card-header {
    border: 1px solid #d0314973;
}
.testimonial-card__content {
    border-bottom: 1px solid #d0314980;
}

.testimonial-card__client .thumb {
    border: 3px solid #d03149;
}
.testimonial-slider .slick-dots li.slick-active button {
    background-color: #d03149;
}
.team-card:hover {
    background-color: #d03149;
    box-shadow: 0 5px 10px 5px #d0314980;
}
.nav-tabs.custom--style-two .nav-item .nav-link.active {
    border-color: #d03149;
    background-color: #d03149;
}
.nav-tabs.custom--style-two .nav-item .nav-link {
    border: 1px solid #d0314973;
}
.table.style--two thead {
    background-color: #d03149;
}
.table.style--two tr th, .table.style--two tr td {
    border-top-color: #d0314940;
}
.table.style--two {
    box-shadow: 0 5px 5px 0 #d0314940;
}
.border-top-1 {
    border-top: 1px solid #d0314980;
}
.cta-wrapper {
    box-shadow: 0 3px 15px #d0314980;
}
.brand-item {
    border: 2px solid #d0314980;
}
.blog-card {
    box-shadow: 0 0 0px 2px #d03149cc;
}
.form-control {
    border: 1px solid #d0314973;
}
.form-control:focus {
    box-shadow: 0 2px 5px #d0314980;
    border-color: #d03149;
}
.footer {
    border-top: 2px solid #d0314980;
}
.scroll-to-top {
    background-color: #d03149;
}
.subscribe-wrapper {
    box-shadow: 0 0 10px #d0314980;
}
.choose-card {
    background-color: #00000080;
}
.pagination li .page-link {
    border: 1px solid #d0314973;
}
a:hover {
    color: #d03149;
}
.contact-item {
    box-shadow: 0 5px 15px #d0314980;
}
.account-card {
    box-shadow: 0 0 10px 2px #d0314973;
}
.account-card__header {
    padding: 50px 30px;
    border-bottom: 2px solid #d0314980;
}
input[type="text"]:read-only, input[type="email"]:read-only, input[type="text"]:disabled, input[type="email"]:disabled {
    background-color: #d0314959;
}

#phoneInput .letter {
    border: 1px solid #d03149 !important;
}
#phoneInput .letter + .letter {
    border-left: 1px solid #d03149 !important;
}
.card {
    box-shadow: 0 0 15px #d0314980;
    background-color: #000000;
    border: 2px solid #d0314980;
}
.list-group-item {
    border: 1px dashed #d0314954;
}
.input-group-text {
    background-color: #d03149;
    border: 1px solid #d03149;
}
.footer__bottom, .header.menu-fixed .header__bottom, .profit-calculator-wrapper, .work-card__icon , .work-card__icon .step-number, .table.style--two, .brand-item,.blog-card,.account-card,.sidebar .widget{
    background-color: #000000;
}
.sidebar .widget{
    border: 1px solid #d03149;    
}

.header__bottom {
    background-color: #00000080;
}

.cookie__wrapper{
    background: #000000}